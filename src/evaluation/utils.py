# copied from https://github.com/shunzh/Code-AI-Tree-Search/blob/main/eval/utils.py
import pprint
import subprocess
...
def colored_background(r, g, b, text):
    ...
    return (
        f"\033[48;2;{clip_and_int(r)};{clip_and_int(g)};{clip_and_int(b)}m{text}\033[0m"
    )
...
def print_colored_output(char_and_uncertainties):
    ...
    hist, _ = np.histogram(uncertainties, bins=10, range=(0.0, 1.0))
    ...
    for char, uncertainty in char_and_uncertainties:
        if "\n" in char:
            ...
            r = int(
                np.clip(255 * uncertainty, 0, 255)
            )  # clip to make sure it's in range, otherwise simply output black color, which would be hard to debug
