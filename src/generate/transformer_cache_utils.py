# from https://github.com/shunzh/Code-AI-Tree-Search/blob/main/transformer_utils/cache.py
import torch
        ...
        for idx, end_index in enumerate(range(prefix_len, output_len)):
            for batch_idx in range(len(output_ids)):
                ...
                top_k_scores, top_k_tokens = torch.topk(
                    scores[idx][batch_idx], k=self.k, sorted=True
                )
...
class GPTSeqCache:
    ...
    def clear(self, new_state):
        self.cache = {
            ...
            for input_ids, output_ids in self.cache.items()
            if new_state == output_ids[: len(new_state)]
        }
